...
import { fetchItineraryByIdAction, updateItineraryAction } from "@/actions/itinerariesActions";
...
export default function EditItineraryForm() {
  ...
  const [formState, dispatch, isPending] = useActionState(
    async (prevState: FormState, formData: FormData) => {
      return await updateItineraryAction(prevState, itineraryId, formData);
    },
    initialState
  );
  ...
  useEffect(() => {
    async function loadItinerary() {
      const res = await fetchItineraryByIdAction(itineraryId);
      if (res.data) {
  ...
  if (loading) {
    return (
      <div className="text-center py-10 text-gray-500">Loading itinerary data...</div>
    );
  }

  if (!itinerary) {
    return <p className="text-center text-red-500">Itinerary not found.</p>;
  }
          ...
          <form action={dispatch} className="space-y-6 max-w-2xl mx-auto">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                defaultValue={itinerary.day_start}
              />
              {errorFor("day_start") && (
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                min={1}
                className="border-none shadow-sm bg-gray-50 dark:bg-gray-700"
                defaultValue={itinerary.day_end || ""}
              />
              {errorFor("day_end") && (
            ...
            <CardFooter className="flex justify-end border-none">
              <Button type="submit" disabled={isPending}>
                ...
                {isPending ? "Updating..." : "Update Itinerary"}
              </Button>
            </CardFooter>
          </form>
        </CardContent>
